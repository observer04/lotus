...
test("CYC-016 cycle schema accepts declared outcomes only",()=>{
  const base={schemaVersion:1,cycleId:"c",startedAt:new Date().toISOString(),finishedAt:new Date().toISOString(),tier:0,startCommit:"a",endCommit:"b",initialSignature:"sig",finalSignature:null,signatureHistory:["sig",null],attemptHistory:[{}],attempts:1,outcome:"green",reason:"all_gates_passed",durationSec:1,harnessDurationSec:1,invocationWaitSec:0,filesChanged:[],attemptedPaths:[],discardedFailureIds:[],dyad:{version:"1.12.0"}};
  assert.equal(assertCycleRecord(base),base);
  assert.throws(()=>assertCycleRecord({...base,outcome:"mystery"}),/invalid cycle outcome/);
  assert.throws(()=>assertCycleRecord({...base,attemptHistory:[]}),/one item per attempt/);
});
...
test("IMP-010 harness schema requires argv-array commands",async()=>{
  ...
  assert.throws(()=>assertHarness({...good,source:{kind:"mystery",identity:"x"}}),/source\.kind/);
  assert.throws(()=>assertHarness({...good,commands:{...good.commands,e2e:[]}}),/commands\.e2e/);
});
