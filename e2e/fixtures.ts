...
export async function configureItem(page: Page, item: string, size = "Small", milk = "Whole") {
  await page.getByTestId(`size-${item}`).selectOption({ label: size });
  await page.getByTestId(`milk-${item}`).selectOption({ label: milk });
}

export async function addItem(page: Page, item: string, size = "Small", milk = "Whole") {
  await configureItem(page, item, size, milk);
  await page.getByTestId(`add-${item}`).click();
}

export function cartLines(page: Page) { return page.getByTestId("cart-line"); }

export async function expectMoney(page: Page, subtotal: string, tax: string, total: string) {
