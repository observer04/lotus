...
test("R4 — Quantity never below 1; decrementing at quantity 1 removes the line", async ({
	page,
}) => {
...
test("R5 — Subtotal, tax, and total computed and displayed per the money rules", async ({
	page,
}) => {
	await addItem(page, "latte", "Small", "Whole");
	await expectMoney(page, "$4.50", "$0.36", "$4.86");
	await expect(cartLines(page).first().getByTestId("line-total")).toHaveText(
		"$4.50",
	);
});

test("R6 — Checkout requires a non-empty name and exactly 10 phone digits; submit disabled until valid", async ({
	page,
}) => {
	await addItem(page, "espresso");
...
test("R7 — On submit, confirmation shows an order number matching /^ORD-\\d{6}$/, an itemized list, and a matching total", async ({
	page,
}) => {
...
test("R8 — Cart survives a page reload, including options and quantities", async ({
	page,
}) => {
	await addItem(page, "espresso", "Large", "Oat");
	await addItem(page, "espresso", "Large", "Oat");
	const stored = await page.evaluate(() =>
		localStorage.getItem("coffee-cart-v1"),
	);
